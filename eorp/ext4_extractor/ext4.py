...
class ext4_xattr_entry(ext4_struct):
    ...
    @property
    def _size(self):
        return 4 * ((ctypes.sizeof(type(self)) + self.e_name_len + 3) // 4)  # 4-byte alignment
...
class ext4_xattr_ibody_header(ext4_struct):
    _fields_ = [("h_magic", ctypes.c_uint)]  # 0x0, Must be 0xEA020000
    ...
    def read_struct(self, structure, offset, platform64=None):
        ...
        if hasattr(structure, "_from_buffer_copy"):
            return structure._from_buffer_copy(
                raw, platform64=platform64 if platform64 != None else self.platform64
            )
        else:
...
class Inode:
    ...
    def get_inode(self, *relative_path, decode_name=None):
        ...
        for i, part in enumerate(relative_path):
            ...
            file_name, inode_idx, file_type = next(
                filter(lambda entry: entry[0] == part, current_inode.open_dir(decode_name)),
                (None, None, None),
            )
    ...
    def is_in_use(self):
        ...
        inode_usage_bitmap_offset = (
            self.volume.group_descriptors[group_idx].bg_inode_bitmap * self.volume.block_size
        )
            ...
            while nodes.qsize() != 0:
                ...
                if header.eh_depth != 0:
                    ...
                    for extent in extents:
                        mapping.append(
                            MappingEntry(extent.ee_block, extent.ee_start, extent.ee_len)
                        )
    ...
    def xattrs(
        ...
        if check_inline and inline_data_length > ctypes.sizeof(ext4_xattr_ibody_header):
            ...
            try:
                for xattr_name, xattr_value in self._parse_xattrs(
                    inline_data[offset:], 0, prefix_override=prefix_override
                ):
                    yield (xattr_name, xattr_value)
            except:
                pass
        ...
        if check_block and self.inode.i_file_acl != 0:
            ...
            for xattr_name, xattr_value in self._parse_xattrs(
                xattrs_block[offset:], -offset, prefix_override=prefix_override
            ):
                yield (xattr_name, xattr_value)
...
class BlockReader:
    ...
    def read(self, byte_len=-1):
        ...
        blocks = [
            self.read_block(i) for i in range(start_block_idx, end_block_idx - start_block_idx + 1)
        ]
...
class Tools:
    ...
    def list_dir(
        ...
        # Print directory
        entries = (
            inode.open_dir(decode_name)
            if sort_key is None
            else sorted(inode.open_dir(decode_name), key=sort_key)
        )
